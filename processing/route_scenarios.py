...
def build_cost_surface_for_scenario(
    ...
    valid = (
        np.isfinite(adequability)
        & np.isfinite(slope)
        & np.isfinite(curvature_horizontal)
        & np.isfinite(curvature_vertical)
        & np.isfinite(risk)
    )
    ...
    final_cost = (
        base_cost
        + float(scenario.get("slope_weight", 0)) * slope_cost
        + float(scenario.get("curvature_weight", 0)) * curvature_cost
        + float(scenario.get("risk_weight", 0)) * risk_cost
        + float(scenario.get("distance_bias", 0))
    ).astype(np.float32)
...
def rank_route_scenarios(rows):
    ...
    def general_score(row):
        ...
        score = (
            0.30 * cost_norm.get(scenario, 0.0)
            + 0.20 * risk_norm.get(scenario, 0.0)
            + 0.20 * mean_slope_norm.get(scenario, 0.0)
            + 0.15 * max_slope_norm.get(scenario, 0.0)
            + 0.15 * distance_norm.get(scenario, 0.0)
        )
...
def create_contrasting_route_dataset(rows=200, cols=140):
    ...
    safe_corridor = (
        ((xx >= 8) & (xx <= 20) & (yy >= start_rc[0]) & (yy <= 152))
        | ((yy >= 145) & (yy <= 160) & (xx >= 14) & (xx <= cols - 20))
        | ((xx >= cols - 24) & (xx <= cols - 8) & (yy >= 150) & (yy <= end_rc[0]))
    )
    transition_corridor = (
        ((xx >= 18) & (xx <= 42) & (yy >= 70) & (yy <= 118))
        | ((xx >= 96) & (xx <= 124) & (yy >= 80) & (yy <= 132))
    )
